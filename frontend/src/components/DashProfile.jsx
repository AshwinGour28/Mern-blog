...
export default function DashProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      ...
      const res = await fetch(`http://localhost:3000/backend/user/update/${currentUser._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handelSignout = async () => {
    try {
      const res = await fetch("http://localhost:3000/backend/user/signout", {
        method: "POST",
      });
      const data = await res.json();
      if(!res.ok){
  ...
  return (
    <div className='max-w-lg mx-auto p-3 w-full'>
      ...
      <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
        ...
        <img src={imageFileUrl || currentUser.profilePicture} alt='user' className={`rounded-full w-full h-full object-cover border-8 border-[lightgray] ${imageFileUploadingProgress && imageFileUploadingProgress < 100 && 'opacity-60'}`}></img>
        </div>
        ...
        <Button type='submit' outline gradientDuoTone='purpleToPink' disabled={loading || imageFileUploading}>
