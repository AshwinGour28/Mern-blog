...
export default function UpdatePost() {
    ...
    useEffect(()=>{
        try {
            ...
            const res = await fetch(`http://localhost:3000/backend/post/getposts?postId=${postId}`);
    ...
    const handleSubmit = async(e)=>{
        e.preventDefault();
        try {
            const res = await fetch(`http://localhost:3000/backend/post/updatepost/${formData._id}/${currentUser._id}`, {
                method: 'PUT',
                credentials: 'include',
                headers: {
    ...
    <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-4 sm:flex-row justify-between'>
            ...
            <Select onChange={(e)=>setFormData({...formData, category: e.target.value})} value={formData.category}>
                ...
                <option value="reactjs">React.sj</option>
                <option value="nextjs">Next.sj</option>
            </Select>
        </div>
